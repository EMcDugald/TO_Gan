...
def make_condition_key(idx, bc_arr, load_arr, conditioning_mode, spatial_edges=None):
    ...
    if conditioning_mode == 'fine':
        ...
        key = f'{conditioning_mode}_idx{idx}_{bc_str}_{load_pt_str}_{load_vec_str}'
        ...
        load_vec_str = f'loadVec({load_vec[0]:.4f},{load_vec[1]:.4f},{load_vec[2]:.4f})'
        key = f'{conditioning_mode}_idx{idx}_{bc_rng}_{bc_bins}_{load_bins}_{load_vec_str}'
    else:
...
def plot_voxel_with_conditions(binary_arr, save_path, title='', bc_arr=None, load_arr=None,
                               conditioning_mode='fine', spatial_edges=None):
...
def main():
    parser = argparse.ArgumentParser(description='Visualize voxel samples with BC/load overlays and condition-key filenames.')
