...
def append_label_to_condition(cond_vec, label_val):
    return np.concatenate([
        np.asarray(cond_vec, dtype=np.float32),
        np.array([float(label_val)], dtype=np.float32)
    ]).astype(np.float32)
...
def generate_voxels_mass_conditioned_with_label_in_cond(
    shape_tuple,
    n_samples,
    topo,
    shapes,
    ...
    loads,
    outdir='./',
    conditioning_mode='fine',
    n_spatial_bins=10,
    positive_if='low_mass',
    rng_seed=0,
    mass_quantile=None
):
    ...
    for idx in selected:
        ...
        if conditioning_mode == 'fine':
            base_cond_vec = make_fine_condition_vector(bcs[idx], loads[idx], max_bc_points=max_bc_points)
            load_pt = parse_load_point(loads[idx])
            cond_str = (
                'fine_'
                + f'label_{label_val}_'
            ...
            base_cond_vec = make_coarse_condition_vector(
                bcs[idx], loads[idx], spatial_edges, max_bc_points=max_bc_points, include_load_vector=True
            )
            load_pt = parse_load_point(loads[idx])
            cond_str = (
                f'coarse_label_{label_val}_bcCount_{len(bc_pts)}'
        ...
        cond_vec = append_label_to_condition(base_cond_vec, label_val)
    ...
    fname = (
        f'{len(results)}_condLabel_voxels_'
        f'{shape_tuple[0]}x{shape_tuple[1]}x{shape_tuple[2]}_'
        f'{conditioning_mode}_bcLoad_massLabelInCond_'
    ...
    np.savez(
        ...
        base_cond_dim=int(len(base_cond_vec)) if len(results) > 0 else -1,
        label_appended_to_condition=True,
        label_index_in_condition=int(len(base_cond_vec)) if len(results) > 0 else -1,
        ...
        cond_description=(
            'fine: padded BC xyz coords + BC mask + BC count + load xyz + load dir + mass label bit; '
            'coarse: BC xyz min/max ranges + padded BC per-point xyz bins + BC mask + BC count + '
            'load xyz bins + load dir + mass label bit'
        ),
        label_mode='mass_label_appended_to_condition',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate mass-labeled voxel training data with BC/load conditioning and append the binary mass label into the condition vector.'
    )
