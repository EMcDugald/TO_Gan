...
OUTPUT_ROOT = "/home/u26/emcdugald/TO_Gan/TO_3D_data_scratch/octant_mass_catalog_323232_v2"
...
def plot_voxel(binary_arr, save_path, title="", bc_arr=None,
    ...
    # Keep axes in voxel coordinates
    ax.set_xlim(0, nx)
    ax.set_ylim(0, ny)
    ax.set_zlim(0, nz)

    # -----------------------------------
    # Octant overlay: three mid-planes
    # -----------------------------------
    if show_octants:
    ...
    if show_bcs and bc_arr is not None:
        ...
        if bc.ndim == 2 and bc.shape[1] >= 3:
            ...
            # Normalize BC cloud exactly as in bc_octant_code
            mins = pts.min(axis=0)
            maxs = pts.max(axis=0)
            spans = np.where((maxs - mins) > 0, maxs - mins, 1.0)
            norm = (pts - mins) / spans

            # Map normalized coords into voxel coordinates
            bcx = norm[:, 0] * (nx - 1)
            bcy = norm[:, 1] * (ny - 1)
            bcz = norm[:, 2] * (nz - 1)
