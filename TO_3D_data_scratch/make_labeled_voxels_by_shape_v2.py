...
def generate_voxels_for_shape(shape_tuple, n_samples, topo, shapes, threshold=0.0, outdir='./'):
    ...
    # NEW: compute positive / negative counts
    labels = [lbl for (_, _, lbl) in results]
    n_pos = sum(labels)
