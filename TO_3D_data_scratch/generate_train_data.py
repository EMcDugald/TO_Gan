...
def make_coarse_condition_vector(bc_arr, load_arr, spatial_edges, include_load_vector=True):
    bc_pts = parse_bc_points(bc_arr)
    load_pt = parse_load_point(load_arr)
    load_vec = parse_load_vector(load_arr)
    ...
    bc_bin_features = []
    for coord in bc_pts:
        bc_bin_features.extend([
            bin_index(coord[0], x_edges),
            bin_index(coord[1], y_edges),
            bin_index(coord[2], z_edges),
        ])
    ...
    cond_parts = [
        ...
        bc_y_rng,
        bc_z_rng,
        np.asarray(bc_bin_features, dtype=np.float32),
        load_xyz_bins,
    ]
...
def generate_voxels_mass_conditioned(
    shape_tuple,
    n_samples,
    topo,
    shapes,
    ...
    for i in indices:
        ...
        all_bc_pts.append(parse_bc_points(bcs[i]))
    ...
    for idx in selected:
        ...
        if conditioning_mode == 'fine':
            cond_vec = make_fine_condition_vector(bcs[idx], loads[idx])
            load_pt = parse_load_point(loads[idx])
            cond_str = (
                'fine_'
                + 'bc_' + '_'.join([f'({p[0]:.4f},{p[1]:.4f},{p[2]:.4f})' for p in parse_bc_points(bcs[idx])])
                + f'_load_({load_pt[0]:.4f},{load_pt[1]:.4f},{load_pt[2]:.4f})'
            )
        elif conditioning_mode == 'coarse':
            cond_vec = make_coarse_condition_vector(bcs[idx], loads[idx], spatial_edges, include_load_vector=True)
            bc_pts = parse_bc_points(bcs[idx])
            load_pt = parse_load_point(loads[idx])
            cond_str = (
                f'coarse_bcx_[{bc_pts[:,0].min():.4f},{bc_pts[:,0].max():.4f}]'
    ...
    np.savez(
        ...
        cond_description=(
            'fine: 3 BC xyz coords + load xyz + load dir; '
            'coarse: BC xyz min/max ranges + BC per-point xyz bins + load xyz bins + load dir'
        ),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate mass-labeled voxel training data with BC/load-only conditioning.')
    ...
    type=float,
    default=None,
    help='If provided, use this quantile in [0,1] as the mass threshold instead of automatic valley detection.'
)
