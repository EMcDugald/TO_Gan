...
DIFFUSION_TRAINER_MODULE = "v0709_diffusion_trainer"
...
def get_mass_meta(meta):
    ...
    return {
        ...
        "vf_mode": str(meta["vf_mode"]) if "vf_mode" in meta else "unknown",
...
def decode_condition_fields(cond_vec, cond_slices):
    ...
    for key in [
        ...
        if key in cond_slices:
            ...
            if key in ("bc_count", "label", "volume_fraction"):
...
def evaluate_condition_consistency(label_val, cond_label_bit, volume_fraction, cond_vf_value, meta):
    mass_info = get_mass_meta(meta)
    if mass_info is None:
        return None

    cutoff = float(mass_info["mass_cutoff_value"])
    positive_if = str(mass_info["positive_if"])

    vf_ref = cond_vf_value if cond_vf_value is not None else volume_fraction
    if vf_ref is None:
        return {
            "has_vf": False,
            "requested_label": int(label_val) if label_val is not None else None,
            "cond_label_bit": cond_label_bit,
            "vf_value": None,
            "cutoff": cutoff,
            "positive_if": positive_if,
            "vf_implied_label": None,
            "label_matches_vf_rule": None,
        }

    if positive_if == "low_mass":
        vf_implied_label = 1 if float(vf_ref) <= cutoff else 0
    ...
    if cond_label_bit is not None:
        requested_label = int(round(float(cond_label_bit)))
    elif label_val is not None:
        requested_label = int(label_val)

    matches = None if vf_implied_label is None or requested_label is None else bool(vf_implied_label == requested_label)

    return {
        "has_vf": True,
        "requested_label": requested_label,
        "cond_label_bit": cond_label_bit,
        "vf_value": float(vf_ref),
        "cutoff": cutoff,
        "positive_if": positive_if,
        "vf_implied_label": vf_implied_label,
        "label_matches_vf_rule": matches,
    }
...
def select_indices_by_subset(y, sample_idx, n_random, rng_seed, subset="all"):
    ...
    if subset == "positive_only":
    ...
    elif subset == "negative_only":
        allowed = np.where(y == 0)[0]
        single_mode = "single_negative"
        random_mode = "random_negative"
    elif subset == "all":
        ...
        raise ValueError(
            f"Unsupported subset {subset!r}; expected 'all', 'positive_only', or 'negative_only'"
        )
...
def run_diffusion_sampler(
    ...
    with torch.no_grad():
        for sample_idx in selected_indices:
            voxel_arr = X[sample_idx]
            cond_vec = C[sample_idx]
            ...
            if cond_slices is not None:
                if "label" in cond_slices:
                    s0, s1 = cond_slices["label"]
                    if s1 > s0:
                        cond_label_bit = float(np.asarray(cond_vec[s0:s1])[0])
                if "volume_fraction" in cond_slices:
                    s0, s1 = cond_slices["volume_fraction"]
                    if s1 > s0:
                        cond_vf_value = float(np.asarray(cond_vec[s0:s1])[0])

            condition_consistency = evaluate_condition_consistency(
                label_val=label_val,
                cond_label_bit=cond_label_bit,
                volume_fraction=volume_fraction,
                cond_vf_value=cond_vf_value,
                meta=meta,
            )
...
def main():
    ...
    parser.add_argument("--data-path", type=str, required=True,
                        help="Path to the diffusion train-data .npy file.")
    parser.add_argument("--meta-path", type=str, default=None,
                        help="Optional explicit meta .npz path; if omitted, inferred from data-path.")
    parser.add_argument("--checkpoint-path", type=str, required=True,
                        help="Path to a diffusion checkpoint .pth file.")
    parser.add_argument("--outdir", type=str, required=True,
                        help="Output directory for PNGs + arrays + JSON summary.")

    parser.add_argument("--img-size", type=int, default=32,
                        help="Cubic voxel size, must match training.")
    ...
    parser.add_argument("--mode", type=str, default="X0",
                        help="Diffusion mode; sampler currently assumes X0-compatible checkpoint.")
    ...
    parser.add_argument("--subset", type=str,
                        ...
                        default="all",
                        help="Pool used to choose reference conditions.")
