...
# -------------------------
# Models (copied from training script)
# -------------------------
...
def main():
    # 1) Paths and hyperparameters (match training)
    data_path = "/xdisk/hdb/emcdugald/to_cond_gan/train_data/323232/10000_labeled_voxels_32x32x32_medianScore.npy"
    ckpt_path = "/xdisk/hdb/emcdugald/to_cond_gan/checkpoints_323232_10k/..."  # <-- point to ckpt_best.pt or ckpt_final.pt
    ...
    max_num_conditions = 20  # how many distinct conditions to sample over

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("device:", device)

    # 2) Load training data just to recover shape and condition vectors
    ...
    # restrict to positives if you like
    pos_mask = (labels == 1)
    ...
    # Option: choose a set of representative condition vectors
    # Here we just randomly choose at most max_num_conditions distinct rows
    ...
    with open(catalog_path, "w") as f_txt:
        f_txt.write("# cond_idx  sample_idx  filename                  condition_vector\n")

        for ci, cond_vec in enumerate(cond_keys):
            ...
            for si in range(samples_per_cond):
                ...
                vox = (fake[0, 0] > 0).astype(np.float32)

                filename = f"cond{ci:03d}_s{si:03d}.png"
                save_path = os.path.join(out_dir, filename)
                plot_voxel_grid_3d(vox, title=f"cond {ci}, sample {si}", save_path=save_path)

                cond_str = " ".join(f"{v:.6f}" for v in cond_vec)
                f_txt.write(f"{ci:03d}  {si:03d}  {filename:<24}  {cond_str}\n")
