...
def parse_args():
    ...
    parser.add_argument("--train_subset",default="all",choices=["all", "positive_only", "negative_only"],type=str,)
...
def make_loaders_by_subset(
    ...
    if subset == "all":
        allowed_indices = np.arange(len(dataset))
    elif subset == "positive_only":
        allowed_indices = np.where(dataset.y == 1)[0]
    elif subset == "negative_only":
        allowed_indices = np.where(dataset.y == 0)[0]
    else:
    ...
    if nsamples is not None and nsamples > 0:
        nsamples = min(int(nsamples), len(allowed_indices))
        chosen_indices = rng.choice(allowed_indices, size=nsamples, replace=False)
        chosen_indices = np.asarray(chosen_indices, dtype=np.int64)
    else:
        chosen_indices = allowed_indices.copy()
        nsamples = len(chosen_indices)
    ...
    train_indices = chosen_indices[:n_train].tolist()
    val_indices = chosen_indices[n_train:].tolist()
    ...
    selected_y = dataset.y[chosen_indices]
    n_pos = int((selected_y == 1).sum())
    n_neg = int((selected_y == 0).sum())

    print(f"Training subset mode: {subset}")
    print(f"Selected {len(chosen_indices)} samples total")
    print(f"Label counts in selected set: pos={n_pos}, neg={n_neg}")
    print(f"Train/val split: {n_train}/{n_val}")

    return dataset, train_loader, val_loader, {
        "subset": subset,
        "n_selected": int(len(chosen_indices)),
        "n_positive": n_pos,
        "n_negative": n_neg,
...
def save_sample_batch(model, dataset, model_dir, epoch, config, device,
    ...
    # Sample conditions from the configured training dataset
...
def main():
    ...
    vf_tag = f"vf-{meta_info['vf_mode']}"
    subset_tag = f"subset-{config['train_subset']}"
    extra_tag = f"_{config['tag']}" if config["tag"] else ""
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")

    run_name = (
        f"{cond_tag}_{mass_tag}_{vf_tag}_{subset_tag}_"
    ...
    dataset, train_loader, val_loader, split_info = make_loaders_by_subset(
    ...
    with open(model_save_dir / "hparams.yml", "w") as f:
        yaml.dump({**config, **meta_info, **split_info, 'meta_path': str(meta_path)}, f)
