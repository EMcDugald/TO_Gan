...
class CondVoxelDataset(Dataset):
    def __init__(self, filepath):
        ...
        if len(self.sample_infos) > 0:
            print("Sample info keys (first item):", list(self.sample_infos[0].keys()))
        ...
        if (
            ...
            with open(txt_path, 'w') as f_txt:
                ...
                for i in range(n_vis_samples):
                    ...
                    title = f'Fake sample {i} (epoch {current_epoch}) | {mode_tag}'
                    plot_voxel_grid_3d(
                        fake[i, 0],
                        title=title,
                        save_path=fig_path,
            ...
            mass_json_path = os.path.join(samples_subdir, f'fake_mass_summary_epoch_{current_epoch:04d}_step_{step+1:08d}.json')
...
def main():
    parser = argparse.ArgumentParser(
        description='Train UNet-based 3D GAN-MDD on BC/load-conditioned, mass-labeled voxel data.'
    )
    ...
    parser.add_argument('--d-every', type=int, default=6)
    ...
    parser.add_argument('--ckpt-every-epochs', type=int, default=5)
    ...
    netG = CondUNetGenerator3D(args.nz, args.ngf, shape3d, cond_dim, cond_channels=8)
    netD = CondDiscriminator3DImproved(args.ndf, shape3d, cond_dim, cond_channels=8, nc=3)
    ...
    with open(os.path.join(checkpoint_dir, 'hparams.txt'), 'w') as f_hp:
        ...
        f_hp.write('conditioning = UNet: broadcast + FiLM in G, broadcast + projection in D\n')
    ...
    with torch.no_grad():
        ...
        with open(txt_path, 'w') as f_txt:
            ...
            for idx in range(args.batch_size):
                ...
                plot_voxel_grid_3d(
                    fake[idx, 0],
                    title=f'Fake sample {idx} | {mode_tag}',
                    save_path=fig_path,
                    bc_points=bc_points,
                    load_point=load_point,
                    load_vec=load_dir,
