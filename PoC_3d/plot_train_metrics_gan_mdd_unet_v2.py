...
def parse_args():
    ...
    p.add_argument('--overlay-name', type=str, default='training_metrics_overlay.png',
                   help='Filename for overlay plot when multiple runs are given')
    return p.parse_args()
...
def load_metrics(checkpoint_dir):
    ...
    # Base metrics (old + new U-Net trainers)
    base_cols = [
        'L_D_real', 'L_D_neg', 'L_D_fake',
        'L_G', 'D_grad_norm', 'G_grad_norm', 'L_div'
    ]
    # New probability metrics in the updated trainer
    prob_cols = [
...
def plot_single_run(df, checkpoint_dir, meta, rolling=0):
    ...
    # ---- Figure 1: main losses + grad norms (as before) ----
    fig, axes = plt.subplots(2, 2, figsize=(13, 10))
    fig.suptitle(f'UNet GAN Training Metrics: {title_suffix}', fontsize=14)

    # D losses vs step
    ax = axes[0, 0]
    for col, label in [('L_D_real', 'D_real'),
                       ('L_D_neg', 'D_neg'),
                       ('L_D_fake', 'D_fake')]:
    ...
    if not df['L_G'].isna().all():
        ax.plot(x_step, maybe_smooth(df['L_G'], rolling),
                color='green', linewidth=2, label='L_G')
    ax.set_xlabel('Step')
    ...
    # Grad norms vs epoch
    ax = axes[1, 0]
    for col, label in [('D_grad_norm', 'D_grad_norm'),
    ...
    # G loss + L_div vs epoch
    ax = axes[1, 1]
    if not df['L_G'].isna().all():
        ax.plot(x_epoch, maybe_smooth(df['L_G'], rolling),
                color='green', linewidth=2, label='L_G')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Generator loss')
    ...
    if not df['L_div'].isna().all():
        ax2 = ax.twinx()
        ax2.plot(x_epoch, maybe_smooth(df['L_div'], rolling),
                 color='red', linestyle='--', alpha=0.7, label='L_div')
    ...
    if not df['L_G'].isna().all():
        ...
        ax.plot(x_step, df['L_G'],
                color='green', alpha=0.25, label='L_G raw')
        ax.plot(x_step, maybe_smooth(df['L_G'], smooth_window),
                color='black', linewidth=2, label=f'L_G smooth ({smooth_window})')
    ax.set_xlabel('Step')
    ax.set_ylabel('Generator loss')
    ...
    # ---- Figure 3: probability diagnostics (new) ----
    prob_cols = [
        ('P_pos_real_pos', 'P(class=1 | real pos)'),
        ('P_neg_real_neg', 'P(class=2 | real neg)'),
        ('P_fake_fake_D',  'P(class=0 | fake, D)'),
    ...
    if has_any_prob:
        ...
        fig3.suptitle(f'UNet GAN Probability Diagnostics: {title_suffix}', fontsize=14)

        # Panel 1: discriminator probabilities vs step
        ax = axes3[0]
        for col, label in [
            ('P_pos_real_pos', 'P(class=1 | real pos)'),
            ('P_neg_real_neg', 'P(class=2 | real neg)'),
            ...
            if col in df.columns and not df[col].isna().all():
                ax.plot(x_step, maybe_smooth(df[col], rolling),
        ...
        # Panel 2: generator success probability vs step
        ax = axes3[1]
        col = 'P_pos_fake_G'
        if col in df.columns and not df[col].isna().all():
            smooth_window = max(rolling, 25) if rolling == 0 else rolling
            ax.plot(x_step, df[col],
                    color='blue', alpha=0.25,
                    label='P(class=1 | fake, G) raw')
            ax.plot(x_step, maybe_smooth(df[col], smooth_window),
                    color='black', linewidth=2,
                    label=f'P(class=1 | fake, G) smooth ({smooth_window})')
        ax.set_xlabel('Step')
        ax.set_ylabel('Probability')
        ax.set_ylim(0.0, 1.0)
    ...
    # ---- Summary file (unchanged except prob columns implicitly available) ----
...
def plot_overlay(runs, overlay_name='training_metrics_overlay.png', rolling=0):
    ...
    for df, ckpt, meta in runs:
        if not df['L_G'].isna().all():
            ax.plot(df['epoch'], maybe_smooth(df['L_G'], rolling),
                    linewidth=1.8, label=meta['run_name'])
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Generator loss')
